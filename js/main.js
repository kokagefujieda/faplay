...
sumsung×
【ipados】
safari△(バックカメラ起動するが手を取得してくれない　handrack.jsの公式サンプルは30FPS前後で使えてる　)
chrome×
他ぷにるおぺらなどなど×
*/
...
const handtrack_status = false; //handtrack.jsの枠等の表示をONにするか
const NipOn = false; //乳首を表示するか
// 最優先の言語だけ取得
...
//*/
if (SOUNDon === false) {
	BGMon = SEon = VOICEon = false
}else{
	//音声、SEが終わってないのにしゃべりだすの禁止
	let insertEND = false;
	let breathEND = false;
	let waterEND = false; 
};
...
const word_debug = false; //中央上に文字情報を載せてデバッグするモード
const fullsc = 0; //フルスクリーン用変数 1でフルスクリーン許可

//_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/
let scene = 0;//現在のゲームシーン
let point; //タップしている座標
let playerSBottomTimeinCamera = 0; //カメラの前で静止している時間
let playersBottom = false; //プレイヤーの動きがストップしたと判定したか否か
let tap = -1; //タップ状態とタップし続けている時間
let cursorAlpha = 0; //カーソル画像のアルファ
let passtime = gametime = 0; //経過時間/ゲームを開始してからの経過時間
let generatesmash = 0; //時々動作を変えるための変数 会心の一撃的な
let TMX = TMY = TMXY = befTMX = befTMY = TMXavg = TMYavg = 0; //handtracktとタップの動きを変換するための変数
// Option & Title & UI オプション・タイトル画面変数----------------
...
let mainbuttonY;//ゲームスタートやシャッターのボタンのY座標
//Model モデルの動き関連の変数----------------
let motionMode = 0; //キャラの動きの状態
let BstRYspeed = BstLYspeed =　ArmRXset = ArmLXset =　ArmRYset = ArmLYset = 0;//胸と腕の位置設定
...
let eyes_partly_open = 0; //目の半開き状態を表現
let tensionup;
let heartcount = 0; //ハートの出た回数　左右の出現位置の振り分けにも使用
let shake_count = 0; //振った回数 万歩計に反映
let zoomdef = zoom_v = zoom_mode = 1.2; //デフォルトのズーム率(キャラクターのサイズ)
let zoom = 1;
...
let movex = movey = 0; //カーソルの毎フレームごとの移動量
let moving = false; //カーソルを動かしているかどうか
//handtrack.js ハンドトラック関連の変数----------------
let BottomX = defBottomX = bottomX = defbottomX = BottomY = defBottomY = bottomY = defbottomY = 0; //シコシコ上下動の上限BottomYと下限bottomYの設定
let MoveDirectionY_ChangeCount = 0; //Y方向の動きを変更した回数=シコシコの回数
let handyY_total = 0; //手を動かしたY座標の蓄積　切り返すとリセット
let handX = befhandX = handyX = 0;//カメラで取得したX軸の手の移動量
let handY = befhandY = handyY = 0;//カメラで取得したY軸の手の移動量
...
let btX = defbtX = 1; //手の左右動の左限と右限の距離
let finishMode = false;//撮影ボタンを押してフィニッシュモード
let shutterbuttonYplus = 2000;
//_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/
...
PIXI.loader //Live 2D model 
	.add('moc', "./assets/chara.moc3", {
		xhrType: PIXI.loaders.Resource.XHR_RESPONSE_TYPE.BUFFER
	})
	.add('texture00', "./assets/texture_00.png")
	.add('chara.model3', "./assets/chara.model3.json", {
		xhrType: PIXI.loaders.Resource.XHR_RESPONSE_TYPE.JSON
	})
	.add('emptymotion', "./assets/empty.motion3.json", {
		xhrType: PIXI.loaders.Resource.XHR_RESPONSE_TYPE.JSON
	})
	.add('physics', "./assets/chara.physics3.json", {
		xhrType: PIXI.loaders.Resource.XHR_RESPONSE_TYPE.JSON
	})
	.load(function(loader, resources) {
		const app = new PIXI.Application(window.innerWidth, window.innerHeight, { /*antialias: false,*/ /* resolution:window.devicePixelRatio || 1, */
			transparent: true,
			resolution: window.devicePixelRatio || 1,
			/*preserveDrawingBuffer: false,antialias: true,autoDensity : true,*/ /*autoResize: true,*/ view: document.getElementById("js-canvas")
		});
		...
		*/
		...
		const breath = PIXI.Sprite.fromImage('./assets/breath.png');
		app.stage.addChild(breath);
		breath.anchor.set(0.5);
		
		const sweattexture = new PIXI.Texture.from('./assets/sweat.png');//しぶき画像テクスチャ化
		const sweatgroup = new PIXI.Container();
		app.stage.addChild(sweatgroup);
		const sweatface = new PIXI.Sprite(sweattexture);
		app.stage.addChild(sweatface);
		sweatface.anchor.set(0.5);
		const sweatarmpitr = new PIXI.Sprite(sweattexture);
		app.stage.addChild(sweatarmpitr);
		sweatarmpitr.anchor.set(0.5);
		const sweatarmpitl = new PIXI.Sprite(sweattexture);
		app.stage.addChild(sweatarmpitl);
		sweatarmpitl.anchor.set(0.5);
		const sweatBstr = new PIXI.Sprite(sweattexture);
		app.stage.addChild(sweatBstr);
		sweatBstr.anchor.set(0.5);
		const sweatBstl = new PIXI.Sprite(sweattexture);
		app.stage.addChild(sweatBstl);
		sweatBstl.anchor.set(0.5);
		...
		const player_shadow = PIXI.Sprite.fromImage('./assets/player_shadow.png');
		...
		const light = PIXI.Sprite.fromImage('./assets/light.jpg');
		app.stage.addChild(light);
		light.blendMode = PIXI.BLEND_MODES.SCREEN;
		light.alpha = 0.5;
		light.visible = false;
		
		const dark = PIXI.Sprite.fromImage('./assets/dark.jpg');
		...
		const sidebutton = PIXI.Sprite.fromImage('./assets/sidebutton.jpg');
		app.stage.addChild(sidebutton);
		sidebutton.alpha = 0;

		const videostartbutton = PIXI.Sprite.fromImage('./assets/RECbutton.png');
		app.stage.addChild(videostartbutton);
		videostartbutton.anchor.set(0.5);
		videostartbutton.interactive = true;
		videostartbutton.on('pointerdown', startVideo);
		...
		//title 説明&注意事項
		const title_body = PIXI.Sprite.fromImage('./assets/title/title_body.png');
		app.stage.addChild(title_body);
		title_body.anchor.set(0.5);

		const title_stick = PIXI.Sprite.fromImage('./assets/title/title_stick.png');
		app.stage.addChild(title_stick);
		title_stick.anchor.set(0.5);

		const title_hand = PIXI.Sprite.fromImage('./assets/title/title_hand.png');
		...
		const title_tablet = PIXI.Sprite.fromImage('./assets/title/title_tablet.png');
		app.stage.addChild(title_tablet);
		title_tablet.anchor.set(0.5);

		const title_tap = PIXI.Sprite.fromImage('./assets/title/title_tap.png');
		...
		const lens = PIXI.Sprite.fromImage('./assets/lens.jpg');
		...
		const soundPath_BGM = PIXI.Sprite.fromImage('./assets/title/option_line.png');
		app.stage.addChild(soundPath_BGM);
		soundPath_BGM.anchor.set(0.5);
		const soundPath_SE = PIXI.Sprite.fromImage('./assets/title/option_line.png');
		app.stage.addChild(soundPath_SE);
		soundPath_SE.anchor.set(0.5);
		const soundPath_VOICE = PIXI.Sprite.fromImage('./assets/title/option_line.png');
		...
		const optionBGM = PIXI.Sprite.fromImage('./assets/title/optionBGM.png');
		app.stage.addChild(optionBGM);
		optionBGM.anchor.set(0.5);
		const optionSE = PIXI.Sprite.fromImage('./assets/title/optionSE.png');
		app.stage.addChild(optionSE);
		optionSE.anchor.set(0.5);
		const optionVOICE = PIXI.Sprite.fromImage('./assets/title/optionVOICE.png');
		app.stage.addChild(optionVOICE);
		optionVOICE.anchor.set(0.5);
		...
		const optionbutton = new PIXI.Sprite(optionbutton_a);
		...
		const option_button_BGM = new PIXI.Graphics();
		...
		const option_button_SE = new PIXI.Graphics();
		...
		const option_button_VOICE = new PIXI.Graphics();
		...
		const help_buttun = PIXI.Texture.fromImage('./assets/title/help_buttun.png');
		const helpbutton = new PIXI.Sprite(help_buttun);
		app.stage.addChild(helpbutton);
		helpbutton.anchor.set(0.5);
		helpbutton.interactive = true;
		helpbutton.on('pointerdown', onClickhelp);

		//frame parts --------------------------------------------------------------------------

		const frametexture = new PIXI.Texture.from('./assets/frame.png');//しぶき画像テクスチャ化
		const framegroup = new PIXI.Container();
		app.stage.addChild(framegroup);		
		const frameTopL = new PIXI.Sprite(frametexture);
		app.stage.addChild(frameTopL);
		frameTopL.anchor.set(0.5);
		const frameTopR = new PIXI.Sprite(frametexture);
		app.stage.addChild(frameTopR);
		frameTopR.anchor.y = 0.5;
		frameTopR.anchor.x = 1;
		frameTopR.scale.x = -1;
		const frameBottomL = new PIXI.Sprite(frametexture);
		app.stage.addChild(frameBottomL);
		frameBottomL.anchor.x = 0.5;
		frameBottomL.anchor.y = 1;
		frameBottomL.scale.y = -1;		
		const frameBottomR = new PIXI.Sprite(frametexture);
		app.stage.addChild(frameBottomR);
		frameBottomR.anchor.y = 1;
		frameBottomR.scale.y = -1;	
		frameBottomR.anchor.x = 1;
		frameBottomR.scale.x = -1;			

		framegroup.addChild(frameTopL, frameTopR, frameBottomL, frameBottomR);
		framegroup.alpha = 0;

		const shutterbutton = PIXI.Sprite.fromImage('./assets/shutterbutton.png');
		app.stage.addChild(shutterbutton);
		shutterbutton.anchor.set(0.5);
		shutterbutton.alpha = 0.7;
		//shutterbutton.interactive = false;
		shutterbutton.on('pointerdown', onClickshutterbutton);
		...
		cur.alpha = cursorAlpha;

		const rippleefect = PIXI.Sprite.fromImage('./assets/tap_effect.png');
...
//次のボイスにすぐに移行したいときは最後の方の吐息っぽい部分をフェードアウトカットで自然かつ早めに終わらせ、次のボイスに移行する

//格ゲー・テイルズ攻撃時みたいにていやーまじまじんけんみたいにボイスの重なりは避ける？
		...
		function pointerUp() { //リタップした時のみ、一度きり(でいい)処理はここ
			zoom_v = zoom_mode = 0;
			tap = -1;
			tapx = tapy = tapnotmovingtime = tapmovex = tapmovey = 0;
			cur.texture = Cur;
			option_button_BGM_tap = option_button_SE_tap = option_button_VOICE_tap = false;
		...
		function onClickshutterbutton() {
			finishMode = true;
		};

		function onClickBGM() {
			if (optionmode === 1) {
				option_button_BGM_tap = true;
			}
		};

		function onClickSE() {
			if (optionmode === 1) {
				option_button_SE_tap = true;
			}
		};

		function onClickVOICE() {
			if (optionmode === 1) {
				option_button_VOICE_tap = true;
			}
		};
		...
		function onClickoption() {
			...
			if (optionmode === -1) {
				optionbutton.texture = optionbutton_a;
			} else {
				optionbutton.texture = optionbutton_b;
			};
			...
			videostartbutton.visible = !videostartbutton.visible;
		...
		app.ticker.add(deltaTimeSeconds => { //()にdeltaTime入れると物理効くようになった!
			...
			model.update(deltaTimeSeconds); //          // //	stats.update();
			...
			shutterbutton.y = mainbuttonY + shutterbuttonYplus;

			//音量ボタンのカーソル追従とアルファ
			if (option_button_BGM_tap === true) {
				option_button_BGM.x = cur.position.x - option_button_BGM.width;
				option_button_BGM.alpha = 0.5;
			} else {
				option_button_BGM.alpha = 1;
			};
			if (option_button_SE_tap === true) {
				option_button_SE.x = cur.position.x - option_button_SE.width;
				option_button_SE.alpha = 0.5;
			} else {
				option_button_SE.alpha = 1;
			};
			if (option_button_VOICE_tap === true) {
				option_button_VOICE.x = cur.position.x - option_button_VOICE.width;
				option_button_VOICE.alpha = 0.5;
			} else {
				option_button_VOICE.alpha = 1;
			};

			//音量ボタン位置の上限下限
			if (option_button_BGM.x < wid / 5) {
				option_button_BGM.x = wid / 5 + wid / 30;
			};
			if (option_button_BGM.x > wid - wid / 5) {
				option_button_BGM.x = wid - wid / 5 - wid / 30;
			};
			if (option_button_SE.x < wid / 5) {
				option_button_SE.x = wid / 5 + wid / 30;
			};
			if (option_button_SE.x > wid - wid / 5) {
				option_button_SE.x = wid - wid / 5 - wid / 30;
			};
			if (option_button_VOICE.x < wid / 5) {
				option_button_VOICE.x = wid / 5 + wid / 30;
			};
			if (option_button_VOICE.x > wid - wid / 5) {
				option_button_VOICE.x = wid - wid / 5 - wid / 30;
			};
			...
			if (passtime === 0) {
				//一度だけ処理でデバッグしたいのはここ?
				zoom = zoomdef;
				//todo handtrack.jsの読み込み
			if( scene > 0 && gametime === 1){
			...
			if (beforex == cur.position.x && beforey == cur.position.y) {
				moving = false
			} //else{moving=true}
			...
			p = target.parameters.values;
				...
				if (gametime > 0) {
					...
					if (gametime === 1) { //ゲーム開始時一度だけの処理
						//タイトル・オプション画像の破棄
						app.stage.removeChild(titles, optionparts, optionbutton, helpbutton, notes);
						p[ArmRY] = p[ArmLY] = -10;
					...
					//移動量を減らす
					if (handyY > 12) {
						handyY = 12 + handyY / 100
					};

					befTMX = TMX; //TMX = Total Move X
					TMX = (handyX / 10 + tapmovex / wid * 250 ) * -1 //*(1.5/*-Math.abs(TMXavg)*/);
					befTMY = TMY; //TMY = Total Move Y
					//    / ウィンドウ高さに応じて移動量も変化
					TMY = ( handyY / (8 + btY / 2) + tapmovey / hei * 250 )  //*(1.5/*-TMYavg*/);
					...
					if (handin_time > -2) { //前提として、手が画面内にある時のみの処理
						...
						if (befTMY < 0 && TMY < 0) {
							...
							if (Math.abs(sameDY) > 3 && Math.abs(handyY_total) > 15 /* && Math.abs(handyY_total) > 6-deltaTimeSeconds*deltaTimeSeconds && btY > 0.5-ToySpeedv*/ ) { //チェンジ時条件 Y軸が同じ方向へ3フレーム以上動いた
								if (MoveDirectionY_ChangeCount % 2 === 0) { //偶数はBottomY,奇数はbottomYの値を取得
									defBottomY = BottomY; //前フレームの値を上限に代入
									defBottomX = BottomX;
									BottomY = (handY + defBottomY) / 2
									BottomY = defBottomY * 0.9 + handY * 0.1; //前フレームとの平均値を上限とする
									BottomX = (handX + defBottomX) / 2;
								} else {
									defbottomY = bottomY; //前フレームの値を下限に代入
									defbottomX = bottomX;
									BottomY = (handY + defbottomY) / 2;
									BottomY = defbottomY * 0.9 + handY * 0.1; //前フレームとの平均値を下限とする
									bottomX = (handX + defbottomX) / 2;
								};

								if (BottomY > bottomY) { //もし下限より上限の方が大きくなってしまったら、値を入れ替える
									[BottomY, bottomY] = [bottomY, BottomY]; //分割代入を使った値の交換 ES2015で可能
								};

								defbtY = btY;
								if( defbtY === void 0){//defbtYがundefinedだったら0を代入
									defbtY = 0;
								}
								
								btY = ((bottomY - BottomY) + defbtY) / 2;
								btY = (btY + btY) / 2; //しつこいくらいに中間の値を取得
								//BtY mini FAP=0-3 camera giri in FAP=15-20?
							}; //チェンジ時条件 END
							if (sameDY !== 0) { //切り替わり時、一度だけの処理
								handyY_total = 0; //handyYの移動量のリセット
								eyeBlink -= 100;  //まばたきに近づける
					...
					if (tap > 0 && handin_time > -1 && handin_count > 0) {
						modelalpha = 0.2;
					} else {
						modelalpha = 1;
					};
					...
					*/
					...
					//カメラの前で静止している時間
					if (TMXY < 0.5) {
						playerSBottomTimeinCamera++;
					} else {
						playerSBottomTimeinCamera = 0;
					};

					///3秒以上放っておくと興奮が静まっていきます。
					if (tap < tensiondowntime * 60 * -1 && 0 <= tension && playerSBottomTimeinCamera > 30) {
						playersBottom = true;
					} else {
						playersBottom = false;
					};
					...
					if (Math.abs(sameDY) === 1 && p[DripAlpha] < 0.1) {
						...
						BstRYspeed = Math.random() * (0.6 - (0.4)) + (0.4);
						BstLYspeed = BstRYspeed + Math.random() * (0.3 - (-0.3)) + (-0.3);
...
if (Math.abs(sameDY) < 2 ) {
					//p[BstRX] += (10- p[BstRX] ) * 0.1;
}else{
	//p[BstRX] += (-10- p[BstRX] ) * 0.1;
};
//p[BstRX] += (p[Y]- p[BstRX] ) * 0.1;
/*
if (sameDY > 0) {
	if(sameDY < 10 ){
	p[BstRX] += (0- p[BstRX] ) * 0.2;
	}else{
	p[BstRX] += (-10- p[BstRX] ) * 0.2;	
	};
};
...
*/
p[BstRX] += (sameDY  - p[BstRX] ) * 0.2;	
p[BstLX] += (sameDY * -1  - p[BstLX] ) * 0.2;
					//*・。*・。*・。*・。*・。*・。*・。*・。*・。*・。*・。*・。*・。*・。
					//胸Y
					//*・。*・。*・。*・。*・。*・。*・。*・。*・。*・。*・。*・。*・。*・。
					...
					p[BstRY] += (p[Y] - p[BstRY]) * BstRYspeed;
					p[BstLY] += (p[Y] - p[BstLY]) * BstLYspeed;

					//腕---------------------------------------
					//兼フィニッシュ演出-----------------------

				
					if (tension > 95 && p[ArmRON] === 0 && p[ArmLON] === 0) {//フィニッシュ
					//ズームがフィニッシュ可能の合図
					zoom　=　zoomdef　*　1.5;
					//ボタンが下からにゅっと出現
					shutterbuttonYplus　+=　(0-shutterbuttonYplus)*0.1;
					//撮影風フレームの描画も併せて出現
					framegroup.alpha  += (1 - framegroup.alpha ) * 0.1;
					
						if(shutterbuttonYplus < 0.1 ){
							shutterbutton.interactive = true;
					...
					if (finishMode === true  ){
							...
							flash.height = hei;
							flash.alpha = 1;
							p[ArmRON] = p[ArmLON] = p[ArmPiece] = 1;
					...
					if (p[Drip2] > 0) {
						//finish
						ArmRYset = ArmLYset = -10;
						ArmRXset = ArmLXset = 10;
						flash.alpha += (0 - flash.alpha ) * 0.1;
						p[Drip2] += (1 - p[Drip2] ) * 0.05;
						zoom = zoomdef * 1.2;

					}else{
						ArmRXset = ArmLXset = (tension/5-10) * -1;
						ArmRYset = ArmLYset = tension/5-10;
						//テンション1
						if (tension > 30 && p[ArmRON] === 1 && p[ArmLON] === 1 && p[ArmPiece] === 0) {//テンションが一定以上になっときに一度だけ処理
							if (passtime % 2 === 0) {
							p[ArmRON] = 0;
							}else{
							p[ArmLON] = 0;
						...
						//テンション2
						if (tension > 80 && p[ArmRON] === 1 || tension > 80 && p[ArmLON] === 1) {
							p[ArmRON] = p[ArmLON] = 0;
						};
						
					};
					//ArmYの移動
						p[ArmRX] += (ArmRXset - p[ArmRX] ) * speedFR/2;
						p[ArmLX] += (ArmLXset - p[ArmLX] ) * speedFL/2;
						p[ArmRY] += (ArmRYset - p[ArmRY] ) * speedFR/2;
						p[ArmLY] += (ArmLYset - p[ArmLY] ) * speedFL/2;
					//━─━─━─━─━─━─━─━─━─━─━─━─━─━─━─━─━─━
					//Z
					//━─━─━─━─━─━─━─━─━─━─━─━─━─━─━─━─━─━
					...
					if (passtime % 1000 - Math.round(tension) * 3 === 0) {
						motionMode = 1; //テンションが上がるほど入りやすくなる
						//目を閉じている時間のランダム化
						if (Math.round(tension) % 3 === 0) {
							shakehead = 90
						};
						if (Math.round(tension) % 7 === 0) {
							shakehead = 120
						};
						shakehead = 60;

						if (Math.round(tension) % 5 === 0) {
							motionMode = 0
						}; //時々キャンセル入る
					...
					if (passtime % 1500 === 0 && tension > 49) {
						motionMode = 3;
						shakehead = 45;
						if (Math.round(tension) % 2 === 0) {
							shakehead = 40;
						};

					};

					if (shakehead > 0　 && motionMode === 3) {
				...
				model.y += ((modeldefy - zoom * 150 + aY + modelDefaultY  + p[Y] * -hei / 300 ) /*+(YY/10)*/ - model.y) * 0.05;
				model.scale.x += ((hei * zoom) - model.scale.x) * 0.1
				...
				if (movexy < 30 && tap < 0) {
					...
					if (p[Level] > 1) {
						...
						if (p[HeadX] > 0) {
							EYE_X_potision = 0 - p[HeadX] / 300 * -1
						}
						if (p[HeadY] < 0) {
							EYE_Y_potision = tension / 300 * p[Y] / 300 - p[HeadY] / 300 * -10
						}
						...
						if (p[HeadX] > 0) {
							EYE_X_potision = 0.7 + p[HeadX] / 300
						}
						if (p[HeadX] < 0) {
							EYE_X_potision = -0.7 - p[HeadX] / 300
						}
				...
				//下に引っ張ると乳首浮き出る  ( p[Y]/20 + 1 )
				if(NipOn === true){
				p[Nip] += ((tension / 100) - ( Math.abs(TMY) / 10 ) - p[Nip]) * 0.5;
				};
				...
				//左右動きに差をつける
				if (passtime % 400 === 0) {
					earRspeed = Math.floor(Math.random() * 10)
				}; //0-9
				if (passtime % 300 === 0) {
					earLspeed = Math.floor(Math.random() * 10)
				}
				...
				if (eye_smile.substring(eye_smile.length - 2, eye_smile.length - 1) % 　7　 === 0) {
					...
					p[EYE_L_SMILE] += (0 - p[EYE_L_SMILE]) * 0.3 + (BstRYspeed - BstLYspeed) / 2;

				} else {
					p[EYE_R_SMILE] += (1 - p[EYE_R_SMILE]) * 0.2;
					p[EYE_L_SMILE] += (1 - p[EYE_L_SMILE]) * 0.3 + (BstRYspeed - BstLYspeed) / 2;
				}
				...
				if (tap > 0) {
					...
					//拡大縮小上下の位置調整
					if (zoom_mode > 0 && gametime > 0 && scene === 0) {//オプションヘルプボタンとの押下位置かぶりなので、ゲームスタート後指定をする
						zoom_v += 0.001;
						if (zoom_mode === 1) {
							zoom -= zoom_v / 10;
						};
				...
				if (eyeBlink < 0) {
					eyeBlink = Math.floor(Math.random() * (200 - 800) + 200)
				};


				player_shadow.y = hei  + p[Y] * -15; //プレイヤーの影(もどき)
				...
				cur.alpha += (cursorAlpha - cur.alpha) * 0.05;
			...
			if (deltaTimeSeconds > 1.2) {
				...
				if (heart2.alpha > 0.99) {
					heart2.visible = false
				};
				if (heart3.alpha > 0.99) {
			...
			if (TMXY > 0.5 && sweatface.alpha < 0.1) {
				...
				sweatface.x = model.x;
				sweatface.y = hei / 4;
				sweatarmpitr.x = model.x - wid / 6;
				sweatarmpitr.y = hei / 2;
				sweatarmpitl.x = model.x + wid / 6;
				sweatarmpitl.y = sweatarmpitr.y;
				sweatBstr.x = model.x - wid / 8;
				sweatBstr.y = hei / 2 + hei / 4;
				sweatBstl.x = model.x + wid / 8;
				sweatBstl.y = sweatBstr.y;
			...
			if (SEon === true) {
				...
				if (sameDY === -1) {
					insertSE.play("down" + waterrandom);
					console.log("down" + waterrandom);
				};

				if (sameDY === 1) {
					insertSE.play("up" + waterrandom);
					console.log("up" + waterrandom);
				};
			...
			if (word_debug === true) {
				//txt.text = "handin_time: " + handin_time + " sameDY: " + Math.round(sameDY) + "BottomY:" + Math.round(BottomY) + "bottomY:" + Math.round(bottomY)
				//txt.text = "handyY: " + handyY + " runD: " + runD + " runDD:" + runDvmodelD + " handin:" + handin ;
				//txt.text = console.log()
			};
		...
		function onPointerMove(eventData) {
			...
			if (eventData.data.global.x > wid) {
				eventData.data.global.x = wid
			};
			if (eventData.data.global.y < 0) {
				eventData.data.global.y = 0
			};
			if (eventData.data.global.y > hei) {
				eventData.data.global.y = hei
			};

			//カーソル動かす
			moving = true; //true
			movex = eventData.data.global.x - cur.position.x;
			movey = eventData.data.global.y - cur.position.y;
			cur.position.x = eventData.data.global.x;
			cur.position.y = eventData.data.global.y;


			beforex = eventData.data.global.x;
			beforey = eventData.data.global.y;
			cursorAlpha = 1;
		...
		function onResize(event) {
			...
			hei = window.innerHeight;
			modeldefx = wid / 2;
			modeldefy = hei / 2 + hei / 3.2;


			console.log("Resize Window !! W:" + wid + "H:" + hei);
			app.view.style.width = wid + "px";
			app.view.style.height = hei + "px";
			...
			optionbutton.x = wid / 2 - wid / 3;
			helpbutton.x = wid / 2 + wid / 3;
			
			optionbutton.y = helpbutton.y = hei - hei / 10;
			optionbutton.width = optionbutton.height = helpbutton.width = helpbutton.height = hei / 10;
			...
			lens.height = hei;
  		    ...
  		    mainbuttonY = videostartbutton.y = hei/2 + hei/4;
			shutterbutton.x = videostartbutton.x = wid/2;
			shutterbutton.width = shutterbutton.height = hei/5;
		
			
			frameTopL.x = wid / 2 - wid / 6;
			frameTopR.x = wid / 2 + wid / 6;
			frameTopL.y = frameTopR.y = hei/2-hei/3;
			
			frameBottomL.x = wid / 2 - wid / 6;
			frameBottomR.x = wid / 2 + wid / 6;
			frameBottomL.y = frameBottomR.y = hei/2+hei/3;
			
			frameTopL.width = frameTopL.height = frameTopR.width = frameTopR.height = hei/9;
			frameBottomL.width = frameBottomL.height = frameBottomR.width = frameBottomR.height = hei/9;
	...
	//_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/￣_/
		...
		//Body 全体角度・位置
		const Y = model.parameters.ids.indexOf("ParamBodyAngleY"); // 身体全体のY(上下)
		const Z = model.parameters.ids.indexOf("ParamBodyAngleZ"); // 身体全体のZ(傾き)
		//Breast 胸 
		...
		const Nip = model.parameters.ids.indexOf("Nip"); //NippleAlpha 乳首の透明度
		const Man = model.parameters.ids.indexOf("ParamMan"); //Stick??? 棒のもっこり具合
		//Shoulder 肩(腕を頭の後ろで組むモード時)
		...
		//Shoulder 腕(腕を脇で曲げるモード時)
		const ArmRX = model.parameters.ids.indexOf("ArmRX"); //右腕X
		const ArmRY = model.parameters.ids.indexOf("ArmRY"); //右腕Y
		const ArmLX = model.parameters.ids.indexOf("ArmLX"); //左腕X
		const ArmLY = model.parameters.ids.indexOf("ArmLY"); //左腕Y
		const ArmRON = model.parameters.ids.indexOf("ArmRON"); //右腕曲げモードのON/OFF
		const ArmLON = model.parameters.ids.indexOf("ArmLON"); //左腕曲げモードのON/OFF
		const ArmPiece = model.parameters.ids.indexOf("ArmPiece"); //手でピース ON/OFF
		//Shoulder 耳
		...
		//表情-------------------------------------------------------------------
		//head 頭
		const HeadX = model.parameters.ids.indexOf("ParamAngleX"); // 顔の左右
		const HeadY = model.parameters.ids.indexOf("ParamAngleY"); // 顔の上下
		const HeadZ = model.parameters.ids.indexOf("ParamAngleZ"); // 顔の傾き
		//EYE 目 
		...
		const Eyehi = model.parameters.ids.indexOf("Eyehi"); //瞳ハイライトの移動(うるうる表現)
		//Brow 眉 
		const BrowR = model.parameters.ids.indexOf("ParamBrowRAngle"); // 右眉
		const BrowL = model.parameters.ids.indexOf("ParamBrowLAngle"); // 右眉
		//Mouth 口
		...
		const Drip2 = model.parameters.ids.indexOf("Drip2"); //挿入汁2
		//Other 他 --------------------------------------------
