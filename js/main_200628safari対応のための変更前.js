...
runDvmodelD = 2;
				...
				err => runDvmodelD = -1//console.log(err)
			)
		}
	})
...
PIXI.loader //Live 2D model 
	...
	.load(function(loader, resources) {
		...
		*/
		...
		const RECbutton = PIXI.Sprite.fromImage('./assets/RECbutton.png');
		app.stage.addChild(RECbutton);
		RECbutton.anchor.set(0.5);
		RECbutton.interactive = true;
		RECbutton.on('pointerdown', onClickRECbutton);
		...
		*/
		...
		function onClickoption() {
			...
			RECbutton.visible = !RECbutton.visible;
	...
	*/
			...
			p = target.parameters.values;
				...
				if (gametime > 0) {
					...
					//    / ウィンドウ高さに応じて移動量も変化
					TMY = ( handyY / (8 + btY / 2) + tapmovey / hei * 250 ) * -1 //*(1.5/*-TMYavg*/);
						...
						if (befTMY < 0 && TMY < 0) {
							...
							if (Math.abs(sameDY) > 3 && Math.abs(handyY_total) > 15 /* && Math.abs(handyY_total) > 6-deltaTimeSeconds*deltaTimeSeconds && btY > 0.5-ToySpeedv*/ ) { //チェンジ時条件 Y軸が同じ方向へ3フレーム以上動いた
								if (sameDY !== 0) { //切り替わり時、一度だけの処理
									eyeBlink = handyY_total = 0; //強制まばたきとhandyYの移動量のリセット
									Brow_potision = 1;
		...
		function onResize(event) {
			...
			lens.y = hei / 2;
  		    ...
  		    mainbuttonY = RECbutton.y = hei/2 + hei/4;
			shutterbutton.x = RECbutton.x = wid/2;
			shutterbutton.width = shutterbutton.height = RECbutton.width = RECbutton.height = hei/5;
